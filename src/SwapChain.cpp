...
CSwapChain::CSwapChain (CDevice &deviceRef, VkExtent2D extent)
		: device {deviceRef}, windowExtent {extent}
{
	init();
}

CSwapChain::CSwapChain (CDevice &deviceRef, VkExtent2D extent, std::shared_ptr<CSwapChain> previous)
...
VkResult CSwapChain::acquireNextImage (uint32_t *imageIndex)
{
	vkWaitForFences(device.GetDevice(), 1, &inFlightFences[currentFrame], VK_TRUE, std::numeric_limits<uint64_t>::max());

	VkResult result = vkAcquireNextImageKHR(device.GetDevice(), swapChain, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphores[currentFrame],  // must be a not signaled semaphore
	                                        VK_NULL_HANDLE, imageIndex);

	return result;
}

VkResult CSwapChain::submitCommandBuffers (const VkCommandBuffer *buffers, uint32_t *imageIndex)
{
